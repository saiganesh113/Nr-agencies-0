  ...
  useEffect(() => {
    if (activeSection === 'orders') {
      const fetchOrders = async () => {
        try {
          const response = await axios.get(`https://nr-agencies-0-api.onrender.com/api/payment/users`);
  ...
  const handleOrderAction = async (transactionId, actionType) => {
    try {
      if (actionType === 'complete') {
        await axios.post(`https://nr-agencies-0-api.onrender.com/api/payment/complete/${transactionId}`);
        setCompletedOrders(prevState => [...prevState, transactionId]);
      } else if (actionType === 'cancel') {
        await axios.post(`https://nr-agencies-0-api.onrender.com/api/payment/cancel/${transactionId}`);
