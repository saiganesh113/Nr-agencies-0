...
useEffect(() => {
  const loadUserData = async () => {
    ...
    try {
      ...
      if (userId) {
        setLoading(true);
        const response = await axios.get(`https://nr-agencies-0-api.onrender.com/api/auth/user/${userId}`, {
...
const handleLogout = () => {
  ...
  const fetchData = async () => {
    try {
      const [
        ...
        axios.get('https://nr-agencies-0-api.onrender.com/api/notifications') // Corrected endpoint
      ]);
  ...
  const handleProceedToPay = async () => {
    try {
      ...
      const response = await axios.post('https://nr-agencies-0-api.onrender.com/api/payment', {
        userid,
        amount,
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      await axios.post('https://nr-agencies-0-api.onrender.com/api/carts', cartItem);
    } catch (error) {
